...
if [ "$testScheme" = "" ] || [ "$unittests" = "" ]; then
    ...
    if [ "$coverageType" = "profdata" -o "$coverageType" = "" ]; then
    	# profdata
    	buildCmd=($XCODEBUILD_CMD test $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64)
        xcode8BuildForTestingCmd=($XCODEBUILD_CMD build-for-testing $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64)
#        xcode8TestCmd=($XCODEBUILD_CMD test-without-building $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64)
        xcode8TestCmd=($XCODEBUILD_CMD test $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator -derivedDataPath "$TESTBUILD_DPATH" ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64 GCC_GENERATE_TEST_COVERAGE_FILES=YES GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES)
    else
    	# Legacy coverage
    	buildCmd=($XCODEBUILD_CMD test $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64)
    	xcode8BuildForTestingCmd=($XCODEBUILD_CMD build-for-testing $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64 GCC_GENERATE_TEST_COVERAGE_FILES=YES GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES)
#        xcode8TestCmd=($XCODEBUILD_CMD test-without-building $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64)
        xcode8TestCmd=($XCODEBUILD_CMD test $buildCmdPrefix -scheme "$testScheme" -configuration Debug -enableCodeCoverage YES -sdk iphonesimulator -derivedDataPath "$TESTBUILD_DPATH" ONLY_ACTIVE_ARCH=YES VALID_ARCHS=x86_64 GCC_GENERATE_TEST_COVERAGE_FILES=YES GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES)
    fi
	...
	echo "{xcode8TestCmd[@] is ${xcode8TestCmd[@]}"
...
#    sed -i "" -e "s/&quot;/\"/g" sonar-reports/TEST-report.xml

	echo -n 'Computing coverage report'

#	if [ "$coverageType" = "profdata" -o "$coverageType" = "" ]; then
	if [ 0 -gt 100 ]; then	# run error when use slather, so use gcovr instead.
...
#		slatherCmd+=( -s --arch x86_64 --configuration Debug)
...
if [ "$oclint" = "on" ]; then
	...
	while read word; do
	    ...
	    runCommand no oclint-json-compilation-database -v $includedCommandLineFlags -- -rc LONG_LINE=$longLineThreshold -disable-rule=ShortVariableName -disable-rule=LongVariableName  -max-priority-1 $maxPriority -max-priority-2 $maxPriority -max-priority-3 $maxPriority -report-type pmd -o sonar-reports/$(echo $word | sed 's/\//_/g')-oclint.xml
