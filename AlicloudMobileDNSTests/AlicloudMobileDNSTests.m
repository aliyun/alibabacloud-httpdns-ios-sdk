//
//  AlicloudMobileDNSTests.m
//  AlicloudMobileDNSTests
//
//  Created by zhouzhuo on 10/9/15.
//  Copyright © 2015 alibaba-inc.com. All rights reserved.
//
...
- (void)setUp {
    ...
    [[ALBBSDK sharedInstance] asyncInit:@"23219835" appSecret:@"f88b70bbb623e5be9f39e2e47e79cb6c" :^{
        NSLog(@"init success");
        initSuccess = YES;
        for (int i = 0; i < 10; i++) {
        ...
        // [self multiThread];
        // HttpDnsServiceProvider * httpdns = [HttpDnsServiceProvider getService];
        // NSArray * hosts = [[NSArray alloc] initWithObjects:@"www.baidu.com", @"oss.aliyuncs.com", nil];
        // [httpdns setPreResolveHosts:hosts];
        // [self parsingHostByHttpdnsAndRequest:@"http://blog.xxyycc.com"];
    } failedCallback:^(NSError *error) {
        NSLog(@"init failed, error: %@", error);
    }];
    while (initSuccess != YES) {
        [NSThread sleepForTimeInterval:1];
    }
#else
    [HttpdnsLog enbaleLog];
