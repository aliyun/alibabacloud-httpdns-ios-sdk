...
# check parameters, usage: ./build.sh framework_id framework_name build_config build_dir [workspace_name]
if [ $# -lt 4 ]; then
  echo "Usage: $0 framework_id framework_name build_config build_dir [workspace_name]"
  echo ""
  echo "Parameters:"
  ...
  echo "  workspace_name  - Workspace名称 (可选，默认使用framework_name)"
  exit 1
fi
...
BUILD_CONFIG=$3
BUILD_DIR=$4
WORKSPACE_NAME=${5:-$FRAMEWORK_NAME}  # 如果没有提供workspace_name，则使用framework_name
...
print_info "Workspace 名称: $WORKSPACE_NAME"
...
build_framework() {
  ...
  xcodebuild -workspace "${WORKSPACE_NAME}.xcworkspace" -configuration "$BUILD_CONFIG" -scheme "$FRAMEWORK_NAME" -sdk "$sdk" $arch_flags build
  ...
  build_settings="$(xcodebuild -workspace "${WORKSPACE_NAME}.xcworkspace" -configuration "$BUILD_CONFIG" -scheme "$FRAMEWORK_NAME" -sdk "$sdk" $arch_flags -showBuildSettings)"
